...
There can be two types of endpoints in the API. Onchain and Offchain. Onchain are endpoints that return the result directly from the RPC node in real time. Due to the specificity of a given blockchain, the response speed may be less effective than the Offchain type.
Offchain: endpoints returning results from a database that processes and parses data from a specific blockchain in real time. We recommend implementing Offchain endpoints in your products to obtain maximum efficiency of the represented data.

...
- Onchain are endpoints that return the result directly from the RPC node in real time. Due to the specificity of a blockchains, the response speed may be less     effective than the Offchain type.
- Offchain: endpoints returning results from a database that processes and parses data from a specific blockchain in real time. We recommend implementing
  Offchain endpoints in your products to obtain maximum efficiency of the represented data, especially for endpoints that need to process and return large amounts of data.
...
Mystra API:</br>
  ...
  Mainet:  <code>comming soon</code></p>
